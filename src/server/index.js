...
const getConfigParameters = async (url) => {
  ...
  const signature = crypto
    .createHash('sha1') // ignore_security_alert_wait_for_fix WEAK_CRYPTO
    .update(
      `jsapi_ticket=${ticket}&noncestr=${NONCE_STR}&timestamp=${timestamp}&url=${url}`
    )
    .digest('hex')
